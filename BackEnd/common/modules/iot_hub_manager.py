...
class _HubStatus(BaseModel):
    """get_hub_status 메서드의 반환 타입을 명시하기 위한 내부 모델"""
    status: str

class _HubBasicInfo(BaseModel):
...
class IoTHubManager:
    ...
    async def add_hub(self, hub_data: HubCreate) -> _HubBasicInfo:
    ...
    async def get_hub_info(self, hub_id: int) -> Optional[_HubBasicInfo]:
        ...
        row = result.first()
        if row:
            return _HubBasicInfo.model_validate(row._mapping)
        return None
    ...
    async def get_hub_status(self, hub_id: int) -> Optional[_HubStatus]:
        ...
        if status_str:
            return _HubStatus(status=status_str)
        else:
            return None
