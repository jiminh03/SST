...
async def update_risk_level(senior_id: int, payload: schemas.RiskLevel, db: AsyncSession = Depends(get_session)):
...
async def collect_ai_error_log(payload: schemas.AIErrorLog, db: AsyncSession = Depends(get_session)):
